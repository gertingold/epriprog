def make_table(datei, hexbase):
    ...
    datei.write("\\noindent\n\\begin{tabular}{%s|}\n\\hline\n" % (cell*8))
    for lsb in range(16):
        datei.write("\\rowcolor{unicodecolor}\n")
        for msb in range(8):
            hexcode = hexbase << 7
            ...
            datei.write("{\\footnotesize\\sffamily U+%04X} %s" %
                        (hexcode, end))
        datei.write("\\rowcolor{utf8color}\n")
        for msb in range(8):
            hexcode = hexbase << 7
            hexcode = hexcode + (msb << 4)
            hexcode = hexcode + lsb
...
def ccodes(datei, hexcode, end):
    ...
    excluded = (0x380, 0x381, 0x382, 0x383, 0x38b, 0x38d, 0x3a2)
    if hexcode in controlcodes:
        datei.write("\\colorbox{cc}{%s} %s" % (controlcodes[hexcode], end))
    elif hexcode in excluded:
        datei.write("\\cellcolor{grau} %s" % end)
    else:
...
\\definecolor{unicodecolor}{rgb}{0.8,0.8,1.0}
\\definecolor{utf8color}{rgb}{0.8,1.0,0.8}
